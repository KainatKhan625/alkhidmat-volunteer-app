...
export default function EditEventScreen({ route, navigation }) {
  ...
  // Load existing event details and pre-fill the form
  useEffect(() => {
    (async () => {
      try {
        ...
        setForm({
          ...
          // Backend date is ISO (e.g. 2026-09-15T00:00:00.000Z), input needs YYYY-MM-DD
          date: event.date ? event.date.split("T")[0] : "",
          seatsRequired: event.seatsRequired ? String(event.seatsRequired) : "",
        });
      } catch (err) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Error",
        err.response?.data?.message || "Could not update event. Please try again."
      );
    } finally {
      setLoading(false);
    }
